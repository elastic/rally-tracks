...
def test_process_inclusive():
    ...
    date_histogram_handler.process(DateTimeValues(min_date=None, max_date=start, duration=None))
...
def test_process_expand_bounds_with_duration():
    ...
    date_histogram_handler.process(DateTimeValues(min_date=None, max_date=start, duration=duration))
...
def test_missing_min():
    ...
    assert (
        rae.value.message == 'Date Histogram aggregation does not have both "min" and "max" for its extended '
...
def test_missing_invalid_bounds():
    ...
    assert (
        rae.value.args[0] == 'Date Histogram aggregation requires epoch milliseconds for its "min" and "max" '
...
def test_invalid_configured_min_max():
    with pytest.raises(TrackConfigError) as err:
        ...
        zealous_min = datetime.datetime(year=2021, month=1, day=1).replace(tzinfo=datetime.timezone.utc)
        DateHistogramHandler(
            {
                "field": "@timestamp",
                "min_doc_count": 0,
    ...
    assert err.value.message == (
        "query-min-date 2021-01-01 00:00:00+00:00 cannot be larger than effective " "query-max-date 2020-12-02 12:33:00+00:00"
    )
...
def test_missing_invalid_range_bounds():
    ...
    assert (
        rae.value.message == '"min" extended bounds of Date Histogram aggregation cannot be greater than "max"'
