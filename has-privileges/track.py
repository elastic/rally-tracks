...
async def has_privileges(es, params):
    ...
    await es.options(basic_auth=("user_" + str(user_id), "password")).security.has_privileges(
        body={
            ...
            "index": [
                {"names": [".watches"], "privileges": ["read", "read"]},
                {"names": [".watcher-history-*"], "privileges": ["read", "read"]},
                {"names": ["logs-*-*"], "privileges": ["read"]},
                {"names": ["traces-*-*"], "privileges": ["read"]},
                {"names": ["metrics-*-*"], "privileges": ["read"]},
                {"names": ["synthetics-*-*"], "privileges": ["read"]},
            ],
            "application": [
                {
