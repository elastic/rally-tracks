...
class TrackIdGenerator:
    ...
    def on_after_load_track(self, track):
        ...
        track_id = hashlib.md5(json.dumps(hash_parameters, sort_keys=True).encode("utf-8")).hexdigest()
        track.selected_challenge_or_default.parameters["track-id"] = track_id
        self.logger.info("Track id is [%s]", track.selected_challenge_or_default.parameters["track-id"])
