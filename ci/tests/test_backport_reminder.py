...
from .resources.cases import GHInteractionCase, PullRequestCase, RepoCase, cases
from .utils import COMMENT_MARKER_BASE
...
def test_get_issue_comments(backport_mod, gh_mock, case: GHInteractionCase):
    ...
    for pr in case.repo.prs:
        ...
        for comment in fetched_comments:
            assert comment in pr.comments
    gh_mock.assert_calls_in_order(*case.expected_order)


@cases(
    pr_that_needs_pending_does_not_get_commented=GHInteractionCase(
        repo=RepoCase(prs=[case_by_number(101)]),
    ),
    pr_does_not_need_pending_and_needs_reminder_gets_one=GHInteractionCase(
    ...
    all_prs_not_need_pending_and_needs_reminder_get_one=GHInteractionCase(
        repo=RepoCase(prs=select_pull_requests(needs_pending=False, needs_reminder=True)),
        routes=[
            *build_gh_routes_comments("GET", select_pull_requests(needs_pending=False, needs_reminder=True)),
            *build_gh_routes_comments("POST", select_pull_requests(needs_pending=False, needs_reminder=True)),
        ],
    ),
    all_prs_not_need_pending_and_has_reminder_does_not_get_one=GHInteractionCase(
        repo=RepoCase(prs=select_pull_requests(needs_pending=False, needs_reminder=False)),
        routes=[
            *build_gh_routes_comments("GET", select_pull_requests(needs_pending=False, needs_reminder=False)),
        ],
    ),
)
def test_remind_logic(backport_mod, gh_mock, case: GHInteractionCase):
    ...
    for pr in case.repo.prs:
        ...
        needs_reminder = backport_mod.pr_needs_reminder(backport_mod.PRInfo(asdict(pr)), threshold)
        assert needs_reminder is (pr.needs_reminder and not pr.needs_pending)

        if needs_reminder:
            backport_mod.post_comment(pr.number, f"{COMMENT_MARKER_BASE}")
