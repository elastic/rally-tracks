...
* cast probes, two-field-extraction probes (`*_2fe_*`), and realistic filter/sort probes (`*_keep_10fields_*`)

#### Data generation

The corpus files for this challenge are generated by:

* `esql/generate_unmapped_data.py`
...
For comparisons, use matched query shapes and densities:

* `*_nullify_*` vs `*_load_*` for direct unmapped-fields cost
* `mapped` vs `mapped_load` for no-unmapped control
* `mapped_source` vs `mapped_load_source` for `_source`-inclusive mapped control
